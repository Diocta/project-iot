// server.js - MQTT to MySQL Bridge (FIXED VERSION)
...
// Timeout to save incomplete data after 10 seconds
...
mqttClient.on("message", async (topic, message) => {
  ...
  try {
    ...
    if (topic === TOPICS.TEMP) {
      ...
      console.log(
        `   🌡️  Temperature updated: ${latestSensorData.temperature}`
      );
    } else if (topic === TOPICS.HUM) {
      latestSensorData.humidity = parseFloat(payload);
      console.log(`   💧 Humidity updated: ${latestSensorData.humidity}`);
    } else if (topic === TOPICS.RAIN) {
      latestSensorData.rain = parseInt(payload);
      console.log(`   🌧️  Rain updated: ${latestSensorData.rain}`);
    } else if (topic === TOPICS.LIGHT) {
      latestSensorData.light = parseInt(payload);
      console.log(`   💡 Light updated: ${latestSensorData.light}`);
    }

    // Log current state of sensor data
    console.log("📊 Current sensor data state:", {
      temp: latestSensorData.temperature !== null ? "✓" : "✗",
      hum: latestSensorData.humidity !== null ? "✓" : "✗",
      rain: latestSensorData.rain !== null ? "✓" : "✗",
      light: latestSensorData.light !== null ? "✓" : "✗",
    });
    ...
    if (
      ...
      console.log(
        "✅ All sensor values present — saving batch:",
        latestSensorData
      );
      try {
        ...
        console.error(
          "❌ saveSensorData threw:",
          e && e.message ? e.message : e
        );
      }
    } else {
      // Set timeout to save incomplete data after 10 seconds
      console.log(
        "⏳ Waiting for complete sensor data... (will auto-save in 10s if incomplete)"
      );
      saveTimeout = setTimeout(async () => {
        if (
          ...
          console.log(
            "⚠️  Timeout reached - saving incomplete sensor data:",
            latestSensorData
          );
          try {
            await saveSensorData(latestSensorData);
    ...
    if (topic === TOPICS.CTRL_LAMP) {
      await logDeviceAction(
        "lamp",
        "All Lamps",
        payload.includes("1") ? "on" : "off",
        "manual"
      );
    } else if (topic === TOPICS.CTRL_GARAGE) {
      const action =
        payload.includes("1") || payload.includes("open") ? "open" : "close";
...
async function saveSensorData(data) {
  try {
    ...
    `;
    const params = [
      data.temperature !== null ? data.temperature : 0,
      data.humidity !== null ? data.humidity : 0,
      data.light !== null ? data.light : 0,
      data.rain !== null ? data.rain : 0,
    ];

    console.log("🔍 Executing DB query:", query.trim());
...
async function logDeviceAction(deviceType, deviceName, action, triggeredBy) {
  try {
    ...
    console.log(
      `✅ Device log saved: ${deviceType} - ${action} (ID: ${result.insertId})`
    );
  } catch (error) {
...
// Test endpoint to manually insert data
...
async function startServer() {
  ...
  app.listen(PORT, () => {
    ...
    console.log(
      `   Test Insert: http://localhost:${PORT}/api/sensor/test (POST)`
    );
  });
}
